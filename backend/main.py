...
# Concurrency guard — max simultaneous ingestion pipelines.
# Each pipeline: OCR (CPU-bound) + embedding (CPU/GPU) + FAISS write (thread).
# 4 concurrent = good throughput on i7/i9; raise to 6-8 on server with more RAM.
# Prevents DB connection exhaustion, memory spikes, and CPU thrashing on batch uploads.
_INGEST_SEMAPHORE = asyncio.Semaphore(4)


async def _guarded_pipeline(**kwargs) -> None:
    """Wraps run_pipeline with semaphore — queues excess tasks instead of crashing."""
